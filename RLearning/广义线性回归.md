...
我们通过几率：$\frac{p}{1-p}$，看$x$如何影响$y$
$\frac{p}{1-p} = e^{w^Tx+b}$
如果$x$增长$1$，odds变更为$e^we^{w^Tx+b}$，变为原来的$e^w$倍，说明$y$增长了
...
### 实例分析
使用`glm`函数
通过`family`指定对$y$的变化，R语言里面有6种(包括不变)
```
...
这里gendermale有了对应$\frac{p}{1-p}$多个$e^{0.28}$倍，每增长一个单位，找他的odds ratio

于是就有了右边这个，回归模型的变量越少越好
...
最好的是$0$，最差的是$L_0$，我们的$L_1$越接近$0$越好
...
还有其他评价指标，混淆矩阵
- 准确率Accuracy，**其实可能没什么用**，$Accuracy = \frac{TP + TN}{TP + TN + FP + FN}$
- 灵敏度Sensitivity、召回率(查全率)Recall$Sensitivity = \frac{TP}{TP + FN}$
...
在ROC曲线（接收者操作特征曲线）上，找到最佳的截断点通常涉及权衡敏感性（True Positive Rate）和特异性（1 - False Positive Rate）之间的关系。最佳截断点(阈值threshold)是在这两个指标中找到一个平衡，以满足问题的具体需求。
...
## 泊松回归
回归函数$log(y) = \alpha + \beta_1x_1 + \beta_2x_2 + \dotsb + \beta_px_p$

响应变量
- 计数数据Count Data，永远大于等于0
- 比例数据Rate Data
...
R代码
```R
glm(breaks ~ wool + tension, data, family = poisson(link = "log"))
```
![](./img/poissonGlmDemo.PNG)
