...
2. `summary(model)`或`(res=summary(model))`生成的`res`对象包含了更详细的回归模型摘要。
   ...
   - **Coefficients**: 这一部分提供了各个预测变量的系数估计。包括了系数的估计值、标准误差、t值和p值。
     ...
     - `Pr(>|t|)`是基于**t统计量得出的p值**，它表示了系数是否统计显著（即不等于零）的概率。
...
## 虚拟变量
虚拟变量（Dummy Variables）是用来表示**分类变量**的一种方法，在回归分析中常用于处理分类或分组数据。当你有一个分类变量，但它**不是连续的数值型变量**时，可以将其转换为虚拟变量，使其能够被回归模型所理解和使用。
...
- `forward$coefficients` 显示了最终模型中的系数估计值。

2. **向后逐步回归分析（Backward Stepwise Regression）**
```r
intercept_only = lm(mpg ~ 1, data = mtcars)
...
- `backward$coefficients` 显示了最终模型中的系数估计值。
